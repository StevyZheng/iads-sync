...
// Config 配置参数
type Config struct {
	AppMaxProc               int           `toml:"app_max_proc"`
	GinMode                  string        `toml:"gin_mode"`
	HttpServerReadTimeout    time.Duration `toml:"http_server_read_timeout"`
	HttpServerWriteTimeout   time.Duration `toml:"http_server_write_timeout"`
	HttpServerMaxHeaderBytes int           `toml:"http_server_max_header_bytes"`

	CasbinPath       string `toml:"casbin_path"`
	CasbinPolicyPath string `toml:"casbin_policy_path"`
